...
import com.tuyoo.action.Actions
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    conf.set("spark.streaming.receiver.maxRate", maxRate);
    //    .setMaster(masterUrl)
    ...
    kafkaStream.map(_._2.split("\t")).filter(fields => { fields.length > 20 && allEvent.contains(fields(6)) })
      .foreachRDD(
        rdd => {
      ...
      case "2" => {
        if (TYCONFIG.loginEvent.contains(line(6))) {
          for (code <- TYCONFIG.loginCode) {
            ...
            sadd(activePipeline, line(3).toLong, key, line(7))
  ...
  def getKey(line: Array[String], logicType: String, code: String, indexes: Array[Int]) = {
    ...
    for (i <- 0 until codeIndexes.length) {
      k = if (codeIndexes(i) == '1') k + "_" + line(indexes(i)) else k + "-1"
    }
    k
  }
